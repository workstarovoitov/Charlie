	...
	void FixedUpdate () {
		if (targets.Length > 0){
			...
			if (!UseWaveAreaCollider) {
				transform.position = new Vector3 (Mathf.Clamp (currentX, MinX, MaxX), currentY, Mathf.Clamp(currentZ, MinZ, currentZ)) + AdditionalOffset;
			} else {
				transform.position = new Vector3 (Mathf.Clamp (currentX, CurrentAreaCollider.transform.position.x + AreaColliderViewOffset, MaxX), currentY, currentZ) + AdditionalOffset;
			}

			//Set cam rotation
			transform.rotation = new Quaternion(viewAngle, 0,0,180f);
		}
	}
